    ...
    $target_dir = "../../Assets/img/";  
     $error=1;
        if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['btn-submit'])){
            ...
            $newbannerimg = round(microtime(true)) . 'bacground.' . end($temp);                           
    ...
    if (!move_uploaded_file($_FILES["img-url"]["tmp_name"], $target_banner)  ) {
     ...
     if($error==1){
        ...
        $sql = "INSERT INTO `tbl_car`(`name`, `brandId`, `modelId`, `seats`, `country`, `decription`, `imgbackground`, `Paint`, `imgurl`, `paintcolor`, `price`, `amount`) VALUES ('".$name."','".$brand."','".$model."','". $seat."','". $place ."','".$decription."','".$imgBacground."','".$color."','".$img ."','".$colorCode."','".$price."','".$amount."') ";
         ...
         header("location:http://localhost/solar2/carShop/admin/manageCar/index.php?message=create");
        }
    }
                ...
                <div class="info-container">
                    <div >
                        ...
                        <h6> Số chỗ ngồi <?php echo ' <span style="color:red">*</span>' ?></h6>
                         <input class="form-control" name="seat" min="0" max="10" type="number">
                    </div>
                    <div>

                 
                   <h6> Giá xe (vnđ) <?php echo ' <span style="color:red">*</span>' ?></h6>
                    <input class="form-control" name="price" min="100000000" type="number">
               </div>
           
              <div >
                   <h6> Số lượng xe (Chiếc) <?php echo ' <span style="color:red">*</span>' ?></h6>
                    <input class="form-control" type="number" placeholder="Điền số lượng xe" min="0" name="amount">
